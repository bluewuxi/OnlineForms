...
import { addInternalUserRole, type InternalRole } from "../lib/internalAccessUsers";
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  try {
    ...
    const data = await addInternalUserRole(userId, role, auth.userId);
    await writeInternalUserActivity({
      userId: data.userId,
      actorUserId: auth.userId,
      eventType: "internal_user.role_added",
      summary: `Role ${role} was added to ${data.email ?? data.username}.`,
      details: { role },
    });
