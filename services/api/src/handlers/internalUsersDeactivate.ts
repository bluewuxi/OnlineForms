...
import { deactivateInternalUser } from "../lib/internalAccessUsers";
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  try {
    ...
    const data = await deactivateInternalUser(userId, auth.userId);
    await writeInternalUserActivity({
      userId: data.userId,
      actorUserId: auth.userId,
      eventType: "internal_user.deactivated",
      summary: `Internal user ${data.email ?? data.username} was deactivated.`,
    });
